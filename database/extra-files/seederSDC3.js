...
// import async.whilst from 'async/whilst';
const bulk = ProductDetail.collection.initializeOrderedBulkOp();
...
let productArr = generateProductNames(productBrandNames, productAdjectives, productSubcategories, 0, 500);
...
// var bulk = db.bulkTest.initializeOrderedBulkOp();
// var bulk = db.collection('ProductDetail').initializeOrderedBulkOp();

const createDocument = (count) => {
  let document = {};
  document.productCategoryId = count;
  document.productCategory = productCategories[createRandomIndex(productCategories)];
  document.productName = productArr[Math.floor(Math.random() * Math.floor(productArr.length))];
  ...
  (callback) => {
    ...
    if ( count % 10 === 0 ) {
      bulk.execute((err,result) => {
        console.log(result);
        console.log("seeded some more");
        callback(err);
      });
      bulk = ProductDetail.collection.initializeOrderedBulkOp();
    } else {
      callback();
    }
  },
